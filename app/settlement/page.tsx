...
import { Server, ShieldCheck, Receipt, X, ZoomIn, Copy, Box, ExternalLink, ShieldAlert, AlertTriangle, Crosshair, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl w-full bg-[#111] border border-zinc-800 shadow-2xl overflow-hidden rounded-md my-8">
      ...
      <div className="p-4 md:p-8">
        ...
        {/* LINE ITEMS (Dynamically Tiered Matrix) */}
        <div className="border border-zinc-800 bg-black mb-8 rounded-sm overflow-hidden">
          <div className="flex justify-between p-3 md:p-4 border-b border-zinc-800 text-[10px] md:text-xs font-bold text-zinc-500 uppercase tracking-widest bg-[#0a0a0a]">
            <span>Cryptographic Payload Breakdown</span>
            <span>Subtotal</span>
          </div>
          
          {invoice.items.map((item, index) => {
            // Mathematical deduction of the tier based on Unit Price
            const unitPrice = item.volume > 0 ? (item.subtotal_usd / item.volume) : 0;
            
            let TierIcon = Box;
            let tierName = "STANDARD PAYLOAD";
            let tierColor = "text-blue-400";
            let bgCol = "bg-blue-500/5";
            let borderCol = "border-zinc-800";

            if (Math.abs(unitPrice - 0.60) < 0.05) {
              TierIcon = CheckCircle2;
              tierName = "GREEN (AUTOMATED EXTRACTION)";
              tierColor = "text-emerald-500";
              bgCol = "bg-emerald-950/10";
              borderCol = "border-emerald-900/30";
            } else if (Math.abs(unitPrice - 2.50) < 0.05) {
              TierIcon = AlertTriangle;
              tierName = "YELLOW (MANUAL REPAIR)";
              tierColor = "text-yellow-500";
              bgCol = "bg-yellow-950/10";
              borderCol = "border-yellow-900/30";
            } else if (Math.abs(unitPrice - 12.00) < 0.05) {
              TierIcon = Crosshair;
              tierName = "RED (VALUE-INTERCEPT AUDIT)";
              tierColor = "text-red-500";
              bgCol = "bg-red-950/10";
              borderCol = "border-red-900/40";
            }

            return (
              <div key={index} className={`p-4 flex justify-between items-center gap-4 border-b last:border-b-0 ${bgCol} ${borderCol}`}>
                <div>
                  <p className={`font-bold text-sm md:text-base leading-tight flex items-center gap-2 ${tierColor}`}>
                     <TierIcon className="w-4 h-4" />
                     {tierName}
                  </p>
                  <div className="flex gap-4 mt-2">
                    <p className="text-[10px] md:text-xs text-zinc-400 font-mono">Vol: <span className="text-white font-bold">{item.volume}</span></p>
                    <p className="text-[10px] md:text-xs text-zinc-400 font-mono">Unit Rate: <span className="text-white">${unitPrice.toFixed(2)}</span></p>
                  </div>
                </div>
                <p className={`text-xl md:text-2xl font-mono font-black ${tierColor}`}>
...
export default function B2BSettlementPage() {
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-slate-300 font-sans flex flex-col items-center justify-start p-4 md:p-6 overflow-y-auto">
      <Suspense fallback={<div className="text-zinc-500 font-mono animate-pulse uppercase tracking-widest text-xs mt-20">Loading Ledger M...</div>}>
        <SettlementContent />
      </Suspense>
    </div>
  );
